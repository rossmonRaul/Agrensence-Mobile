...
import { ObtenerRegistroContenidoDeAgua } from '../../../../servicios/ServicioContenidoAgua';
import { paginationStyles } from '../../../../styles/pagination-styles.styles';
import CustomAlertAuth from '../../../../components/CustomAlert/CustomAlert';

export const ListaContenidoAguaScreen: React.FC = () => {
    ...
    const [contenidoAgua, setContenidoAgua] = useState<any[]>([]);
    ...
    const keyMapping = {
        // 'Finca': 'nombreFinca',
        // 'Parcela': 'nombrParcela',
        'Punto de medición': 'codigo',
        'Fecha Muestreo': 'fechaMuestreo',
        'Contenido Agua en el Suelo': 'contenidoDeAguaEnSuelo',
        'Contenido Agua en la Planta': 'contenidoDeAguaEnPlanta',
        'Metodo de Medicion': 'metodoDeMedicion',
        'Condicion del Suelo': 'condicionSuelo',
        'Estado': 'estado'
    };

    const handleRectanglePress = (idContenidoDeAgua: string, idFinca: string, idParcela: string, idPuntoMedicion :string,  fechaMuestreo: string,
        contenidoDeAguaEnSuelo: string, contenidoDeAguaEnPlanta: string, metodoDeMedicion: string,  condicionSuelo: string, estado: string) => {
        ...
        navigation.navigate(ScreenProps.ModifyWaterContent.screenName, {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setContenidoAgua([]);
                ...
                setParcelas(parcelas);
                //se obtienen los datos de el registro condiciones meteorologicas para despues poder filtrarlos
                const registroContenidoAguaResponse = await ObtenerRegistroContenidoDeAgua();
                //si es 0 es inactivo sino es activo resetea los datos
                const filteredData = registroContenidoAguaResponse.map((item) => ({
                    ...item,
                    estado: item.estado === 0 ? 'Inactivo' : 'Activo',
                }));
                setOriginalApiData(registroContenidoAguaResponse);
                setCurrentPage(1);
    ...
    const currentItems = contenidoAgua.slice(indexOfFirstItem, indexOfLastItem);
    ...
    const handleParcelaChange = (item: { label: string; value: string }) => {
        ...
        if (fincaId !== null) {
            ...
            obtenerRegistroContenidoAguaPorFincaYParcela(fincaId, parcelaId);
        } else {
    ...
    const obtenerRegistroContenidoAguaPorFincaYParcela = async (fincaId: number, parcelaId: number) => {
        try {

            const ContenidoAguaFiltrado = apiData.filter(item => item.idFinca === fincaId && item.idParcela === parcelaId);

            setContenidoAgua(ContenidoAguaFiltrado);
        } catch (error) {
            console.error('Error fetching data:', error);
        }
    };
    ...
    const renderPagination = () => {
        const totalPages = Math.ceil(contenidoAgua.length / itemsPerPage);
        let startPage = 1;
                ...
                <ScrollView style={styles.rowContainer} showsVerticalScrollIndicator={false}>
                    ...
                    <>
                        {currentItems.map((item, index) => (
                            ...
                            <TouchableOpacity key={item.idContenidoDeAgua} onPress={() => handleRectanglePress(
                                item.idContenidoDeAgua, item.idFinca, item.idParcela,item.idPuntoMedicion, item.fechaMuestreo, item.contenidoDeAguaEnSuelo, item.contenidoDeAguaEnPlanta, item.metodoDeMedicion,
                                item.condicionSuelo, item.estado
                            )}>
                                <CustomRectangle
                                key={item.idContenidoDeAgua}
