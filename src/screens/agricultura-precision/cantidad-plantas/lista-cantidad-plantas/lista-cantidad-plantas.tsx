...
import { ObtenerRegistroCantidadDePlantas } from '../../../../servicios/ServicioCantidadDePlantas';
import { paginationStyles } from '../../../../styles/pagination-styles.styles';

export const ListaCantidadDePlantasScreen: React.FC = () => {
    ...
    const handleRectanglePress = (idCantidadDePlantas: string, idFinca: string, idParcela: string, idPuntoMedicion :string, cultivo: string, cantidadPromedioMetroCuadrado: string,
        estado: string) => {
        ...
        navigation.navigate(ScreenProps.ModifyNumberOfPlantsContent.screenName, {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setCantidadDePlantas([]);
                ...
                const registroCantidadDePlantasResponse = await ObtenerRegistroCantidadDePlantas();
                //si es 0 es inactivo sino es activo resetea los datos
                const filteredData = registroCantidadDePlantasResponse.map((item) => ({
                    ...item,
                    estado: item.estado === 0 ? 'Inactivo' : 'Activo',
                }));
                setOriginalApiData(registroCantidadDePlantasResponse);
                setCurrentPage(1);
    ...
    const currentItems = cantidadDePlantas.slice(indexOfFirstItem, indexOfLastItem);
    ...
    const handleParcelaChange = (item: { label: string; value: string }) => {
        ...
        if (fincaId !== null) {
            ...
            obtenerRegistroCantidadDePlantasPorFincaYParcela(fincaId, parcelaId);
        } else {
    ...
    const obtenerRegistroCantidadDePlantasPorFincaYParcela = async (fincaId: number, parcelaId: number) => {
        try {

            const CantidadDePlantasFiltrado = apiData.filter(item => item.idFinca === fincaId && item.idParcela === parcelaId);

            setCantidadDePlantas(CantidadDePlantasFiltrado);
        } catch (error) {
            console.error('Error fetching data:', error);
        }
    };
    ...
    const renderPagination = () => {
        const totalPages = Math.ceil(cantidadDePlantas.length / itemsPerPage);
        let startPage = 1;
    ...
    return (
        <View style={styles.container}>
            <View style={styles.listcontainer}>
                ...
                <AddButtonComponent screenName={ScreenProps.InsertNumberOfPlantsContent.screenName} color={'#274c48'} />
                <View style={styles.textAboveContainer}>
                    <Text style={styles.textAbove} >Lista cantidad de plantas</Text>
                </View>
                ...
                <ScrollView style={styles.rowContainer} showsVerticalScrollIndicator={false}>
                    ...
                    <>
                        {currentItems.map((item, index) => (
                            ...
                            <TouchableOpacity key={item.idCantidadDePlantas} onPress={() => handleRectanglePress(
                                item.idCantidadDePlantas, item.idFinca, item.idParcela,item.idPuntoMedicion, item.cultivo, item.cantidadPromedioMetroCuadrado, item.estado
                            )}>
                                <CustomRectangle
                                key={item.idCantidadDePlantas}
