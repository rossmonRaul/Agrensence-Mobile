...
import { ObtenerReporteOrdenDeCompra } from '../../../servicios/ServicioReporte';
...
export const ReporteOrdenDeCompra:   React.FC = () => {
    ...
    const [ordenCompraExportar, setOrdenCompraExportar] = useState<any[]>([]);
    ...
    const [ordenCompraTotales, setOrdenCompraTotales] = useState<any[]>([]);  
    ...
    const keyMappingTotal = {
        'Orden de Compra Total': 'ordenCompraTotal'
    };
    ...
    const obtenerFlujoCajaPorFinca = async (fincaId: number,) => {
        try {
            ...
            setOrdenCompra(entradaSalidaFiltrado);
        } catch (error) {
            console.error('Error fetching data:', error);
        }
    };
    ...
    const handleDateFilter = async () => {
       ...
       // console.log("DATAEnviar",formData );
        const ordenCompraResponse = await ObtenerReporteOrdenDeCompra(formData);
        //console.log('Entrada salida',ordenCompraResponse);
        setOrdenCompra(ordenCompraResponse);
        setOrdenCompraExportar(ordenCompraResponse);

        setOrdenCompraTotales([]);
        let valorTotal=0;
        ...
        setApiData(ordenCompraResponse);
    ...
    const formatDate = (date: Date) => { // Aquí se crea un objeto Date a partir de la cadena dateString
        ...
        const renderPagination = () => {
            const totalPages = Math.ceil(ordenCompra.length / itemsPerPage);
            let startPage = 1;
        ...
        <View style={styles.listcontainer}>
            ...
            <View style={styles.rowContainer} >
                {!ordenCompraTotales ? (
                    ...
                    <>
                        {ordenCompraTotales.map((item, index) => (
                            <CustomRectangle
                                key={item.ordenCompraTotal}
